...
namespace PragmaOnce.WebAPI.src.Data
{
    public class SeedingData
    {
        ...
        private static int GetRandomNumber(int min = 1, int max = 11)
        {
        ...
        private static List<CandidateProfile> candidates = new List<CandidateProfile>
         {
         };

        private static List<RecruiterProfile> recruiters = new List<RecruiterProfile>
        {
        };
        ...
        public static List<CandidateProfile> CreateCandidateProfiles(IEnumerable<User> users)
        {
            ...
            foreach (var user in users)
            {
                candidates.Add(new CandidateProfile {
                    ...
                    UserId = user.Id,
                    MaxYearsExperience = GetRandomNumber(5, 10),
                    MinYearsExperience = GetRandomNumber(1, 7)
                }
                );
            }
            return candidates;
        ...
        public static List<RecruiterProfile> CreateRecruiterProfiles(IEnumerable<User> users)
        {
            var recruiters = new List<RecruiterProfile>();
            foreach (var user in users)
            {
                recruiters.Add(new RecruiterProfile { Id = Guid.NewGuid(), UserId = user.Id});
            }
            return recruiters;
        }

        public static List<Vacancy> CreateVacancies(IEnumerable<User> users)
        {
            var vacancies = new List<Vacancy>();
            foreach (var user in users)
            {
                vacancies.Add(new Vacancy { Id = Guid.NewGuid(), RecruiterProfileId = user.Id, Title = GetRandomJobTitle() });
            }
            return vacancies;
        }
