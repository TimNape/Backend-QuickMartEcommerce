...
namespace PragmaOnce.Tests.src.xUnitTests.Service.Shop
{
    public class OrderServiceTests
    {
                ...
                OrderItems = new List<OrderItemCreateDto>
        {
            ...
            Assert.Equal(orderCreateDto.OrderItems.Count, (result.OrderItems != null ? result.OrderItems.Count : 0));
        }
        ...
        public async Task UpdateOneAsync_ShouldUpdateAndReturnUpdatedDto()
        {
            ...
            _mockMapper.Setup(m => m.Map(updateDto, existingOrder)).Callback((OrderUpdateDto src, Order dest) => dest.Status = (OrderStatus)src.Status!);
